...
const CharacterImageGuess = () => {
  ...
  useEffect(() => {
    const imagePath = `/animal/${targetCharacter.name.toLowerCase().replace(' ', '')}.jpg`;
  ...
  const handleGuess = () => {
    if (guess === targetCharacter.name) {
      ...
      const newHint = `Wrong guess: "${guess}". Hint: The animal have ${targetCharacter.leg} legs`;
